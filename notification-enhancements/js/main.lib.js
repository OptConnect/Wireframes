...
function doTriggerThings(ELEM) {
	console.log("ELEM:", ELEM);
	const targetItem = _$(ELEM).attr('data-target') || ELEM;
	console.log("starting with targetItem:", targetItem);
	if ( !(_$(ELEM).hasClass('modal')) ) {
		if (targetItem.includes("#") && (_$(targetItem).hasClass('prepend') || _$(targetItem).hasClass('append'))) {
			e.preventDefault();
			...
			console.log("found check item:", targetItem);
			...
			if (ELEM.dataset.label) {
				labels = ELEM.dataset.label.split('|');
			}
			...
			if (groupTotal == groupSelected) {
				_$(targetItem).items.forEach((_item_) => {
					...
					if (ELEM.dataset.label) {
						ELEM.innerHTML = labels[0];
					}
					console.log("item inside grouptotal:", _item_)
				...
				_$(targetItem).items.forEach((_item_) => {
					_item_.checked = true;
					if (ELEM.dataset.label) {
						ELEM.innerHTML = labels[1];
					}
					console.log("_item_: ", _item_)
					if (_$(_item_).hasClass('store')) {
						const itemName = _item_.id;
						const val = _item_.value;
		...
		// does not have "check" or "prepend|append"
		else {
			if (ELEM.tagName !== 'SELECT') {
				// checkboxes
				if (ELEM.tagName === 'INPUT' && ELEM.type === 'checkbox') {
					if (ELEM.checked) {
						// console.log("it's these:", targetItem);
						_$(targetItem).items.forEach((_selector_) => {
				...
				// radios
					...
					const currName = _$(ELEM).attr('name');
					const targetElems = ELEM.dataset.target ? ELEM.dataset.target.split(',').map(item => item.trim()) : [];
					// let isAlreadyActivated = targetElems.some(_selector_ => _$(_selector_).hasClass('activated'));
					
					if (ELEM.checked && !ELEM.dataset.already && ELEM.dataset.target) {
						const currElem = ELEM;
						...
						_$(`[name="${currName}"]`).items.forEach((_elem_) => {
							if (_elem_.dataset.target) {
								...
								// console.log("running each targetElem:", targetElems)
								targetElems.forEach((_selector_) => {
									if (_$(_selector_).hasClass('activated')) {
										_$(_selector_).addClass('activate-out');
										_$(_selector_).removeClass('activated');
...
_$('.trigger').click((e) => {
	// const targetItems = e.target.dataset.target ? e.target.dataset.target.split(',').map(item => item.trim()) : [];
	...
	if (untarget) {
		_$(untarget).items.forEach((_elem_) => {
			if (_$(_elem_).hasClass('activated')) {
				...
				_$(_elem_).addClass('activate-in');
				setTimeout(() => {
					_$(_elem_).addClass('activated');
					_$(_elem_).removeClass('activate-in');
				}, 200);
			}
		});
	}
	console.log("e.target.dataset.select", e.target.dataset.select);
	if (e.target.dataset.select) {
		console.log("it has select things: ", e.target.dataset.select)
		checkItems = e.target.dataset.select;
		_$(checkItems).items.forEach((_elem_) => {
			if (_elem_.checked) {
				_elem_.checked = false;
			}
			else {
				_elem_.checked = true;
				localData[thisPage][_elem_.id] = _elem_.id
				console.log("storing", _elem_.id)
				localData.save();
			}
		});

	}
	doTriggerThings(e.target);
	const targetElems = e.target.dataset.target ? e.target.dataset.target.split(',').map(item => item.trim()) : [];
	console.log("targetElems:", targetElems);
	_$(targetElems).vanilla.forEach((_selector_) => {
		// console.log("_selector_:", _selector_);
		if (_$(_selector_).hasClass('trigger')) {
			// console.log("this target", _selector_, "is also a trigger");
			if (_selector_.includes("#")) {
				_selector_ = [_selector_]
			}
			_$(_selector_).vanilla.forEach((_item_) => {
				const targetItem = _$(_item_).attr('data-target') || _selector_;
				// console.log("targetItem:", targetItem);
				doTriggerThings(targetItem);
				if (_$(targetItem).hasClass('check')) {
					console.log("clicking:", targetItem);
					_$(targetItem).vanilla.click();
				}
			});
		}
	})
...
localData['remove'] = function(PAGE) {
	if (PAGE) {
		localData[thisPage] = {};
		localData.save();
	}
...
_$('#clearLocalStorage').click(() => {
	localData.remove();
});

_$('#clearPageStorage').click(() => {
	localData.remove(thisPage);
});
